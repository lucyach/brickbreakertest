...
def moveBall():
  for ball in balls:
    ...
    if(ball.shouldLoseLife()):
      gs.lives = gs.lives - 1
      for brickNumber in range(90, 99):
        basketOfBricks[brickNumber].setMaxHits(basketOfBricks[brickNumber].getMaxHits() - 1)
      for ball in balls:
        ball.reset()
...
def draw():
  for ball in balls:
    if not gs.hasLost and not gs.hasWon:
      ...
      if(ball.collidesWith(paddle) and not ball.amICollectible):
