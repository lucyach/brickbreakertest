...
#/*******Ball Class**************/
class Ball:
  def __init__(self, isCollectible):
    self.amICollectible = isCollectible
    self.ballX = random.randint(0, widthD)
    self.ballY = heightD/2
    self.ballWidth = 16
    self.speedY = 4
    if not self.amICollectible:
      self.speedX = 4
    else:
      self.speedX = 0
  ...
  def collidesWith(self, brick):
    ...
    if not self.amICollectible:
      ...
      if(pointInRect(self.ballX, self.ballY - self.ballWidth/2, brick.blockX, brick.blockY, brick.blockWidth, brick.blockHeight)):
          print("I am the ball and I have hit a block")
